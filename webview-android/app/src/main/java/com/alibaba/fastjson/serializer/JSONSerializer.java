        ...
        if (writer == null) {
            if (Map.class.isAssignableFrom(clazz)) {
                ...
                for (Class<?> item : clazz.getInterfaces()) {
                    if ("net.sf.cglib.proxy.Factory".equals(item.getName())
                            || "org.springframework.cglib.proxy.Factory".equals(item.getName())) {
                        isCglibProxy = true;
                        break;
                    } else if ("javassist.util.proxy.ProxyObject".equals(item.getName())) {
