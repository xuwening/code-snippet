...
+ (void)runRunLoopThread
{
  @autoreleasepool {
    ...
    //设置当前线程名(pthread默认没有线程名称)
    pthread_setname_np([NSThread currentThread].name.UTF8String);

    // Set up a dummy runloop source to avoid spinning
    
    //iOSrunloop包含3种事件：CFRunLoopSourceRef、CFRunLoopTimerRef、CFRunLoopObserverRef
    ...
    // run the run loop
//    CFTimeInterval sec = ((NSDate *)[NSDate distantFuture]).timeIntervalSinceReferenceDate;
    //意思是获取一个随机的将来的时间，然后再从1970年到这个时间的间隔，所以这个间隔时间很长，不可能真正到这个时间
...
//在单独线程创建js context，并发送通知告知接收者context
//使用者，可以注册该通知获取context，并注入自己的代码（context属于单独线程，需要注意线程互斥问题）
