...
- (void)didReceiveNewContentSizeMultiplier
{
  ...
  dispatch_async(self.methodQueue, ^{
    ...
    if (strongSelf) {
      ...
      //先发通知，让他处也处理该事件（目前只有RCTShadowRawText接收了该事件）
...
//因为UIManager也是组件，所以遵循组件协议，在单独队列中执行
...
//设置view的frame，从这里可以看出，frame更新的是shadowView，而非view
...
//不用实时调用的操作，放在主线程执行的操作，当调用flushUIBlocks时将所有block一次执行
...
{
  //组件中包含所有要导出的，只有一个实例，使用name索引，以RCTView为例，只有一个
  ...
  if (shadowView) {
    ...
    //保存shadowView，一个shadowView对应一个UIView
...
//调用manager中的某个方法，根据reactTag和commandID
