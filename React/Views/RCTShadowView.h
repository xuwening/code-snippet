...
//这是RCT view树的镜像，不是view，主要是属性及样式
//树节点三种状态（生命周期）：未初始化、已经计算、脏数据
//用于渲染性能，如果某节点数据处于`已经计算`，但是上游节点布局更新结果和最后一次相同，则该节点下的子节点不进行重新计算。
//bridge更新边距属性时，有肯能该节点已经属于脏数据，则不用再更新子节点
//bridge每次事务结束，需要重现建立整个view树

//设置view属性时，同时设置shadowView属性
//shadowView保存着css属性
