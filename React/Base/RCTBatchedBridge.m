...
- (void)initModulesWithDispatchGroup:(dispatch_group_t)dispatchGroup
{
  ...
  //这里是临时moduleData保存，扫描导出类完毕后，再统一保存到全局
  ...
  //javaScriptExecutor也是一个RCTBridgeModule，所以也为它建立ModuleData
  //这个模块必须在其他模块访问bridge之前建立
  ...
  //RCTModuleClasses是全局静态变量，里面通过每个类在+load时自动注册到这里
  //为每个moduleClass创建moduleData，并全局索引保存，方便后续查找
  for (Class moduleClass in RCTGetModuleClasses()) {
    ...
    //创建一个moduleData实例，并指向某个导出类，并判断导出类是否需要在主线程执行，是否有导出向量
    //后续初始化时，才真正创建导出类实例
    ...
    if (moduleData.requiresMainThreadSetup || moduleData.hasConstantsToExport) {
      ...
      dispatch_group_async(dispatchGroup, dispatch_get_main_queue(), ^{
        if (weakSelf.valid) {
          ...
          //初始化模块，创建实例，设置bridge、methodQueue
          (void)[moduleData instance];
          
          //导出常量（其实就是保存到模块的_constantsToExport属性中）
...
//给js注入__fbBatchedBridgeConfig的信息
