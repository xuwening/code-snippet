...
//特点： 
//一、从右往左比较 ；  
//二、两种启发规则：坏字符规则、好后缀规则，根据规则决定向右跳跃的距离

//比较开始T与P左对齐，从右往左比较，一旦出现不匹配字符则启用启发规则向后跳跃。
//eg:  T为文本串，P为模式串，m为P的长度，x为不匹配字符
// T:  a b c e c a b e
// P:  a b c a b

//坏字符规则：
//一、不匹配字符x没有在P中出现，则从x开始的m个文本不可能匹配，直接跳m
//二、不匹配字符x出现在P中，则以该字符对齐[P中有多个x，以最右位置为准]
// T:  a b c e c a b e
// P:      a b c a b
...
int main(int argc, char *argv[])
{
	...
	int *skip = MakeSkip(P, strlen(P));
	int *shift = MakeShift(P, strlen(P));
	int ret = BMSearch(T, strlen(T), P, strlen(P), skip, shift);
	if (ret == 1)
